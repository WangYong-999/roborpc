...
simulation_app = SimulationApp({"headless": False, "open_usd": os.getcwd() + "/panda_robotiq85_ycb.usd"})
...
class IsaacSim:
    def __init__(self):
        ...
        my_task = PickPlace(name="panda_85_pick_place", target_position=target_position)
        self.my_world.add_task(my_task)
        self.my_world.reset()
        self.task_params = self.my_world.get_task("panda_85_pick_place").get_params()
        ...
        self._handeye_cam_orientation = Gf.Vec3d(*self._handeye_cam_config['orientation'])
        ...
        self.gripper_velocities = np.nan * np.ones(2)

        robot_rpc = SingleFranka(self)
    ...
    def run(self):
        ...
        while simulation_app.is_running():
            ...
            if self.my_world.is_playing():
                ...
                self.current_gripper_positions = observations[self.task_params["robot_name"]["value"]]["joint_positions"][7:9]

                actions = np.nan * np.ones(15)
                actions[:7] = self.joint_positions
                actions[7:9] = self.gripper_velocities
                self.joint_positions = np.nan * np.ones(7)
                self.gripper_velocities = np.nan * np.ones(2)
