...
class SVOReader:
    ...
    def get_frame_resolution(self):
        if config["droid"]["robot"]["control_mode"] == "evaluation":
            camera_width = config["droid"]["evaluation"]["image_width"]
            camera_height = config["droid"]["evaluation"]["image_height"]
        else:
            camera_width = config["droid"]["collector"]["image_width"]
            camera_height = config["droid"]["collector"]["image_height"]
        camera_info = self._cam.get_camera_information(sl.Resolution(camera_width, camera_height)).camera_configuration
    ...
    def read_camera(self, ignore_data=False, correct_timestamp=None, return_timestamp=False):
        ...
        err = self._cam.grab()
        print(f"err:{err}")
        print(f"zed.get_svo_position():{self._cam.get_svo_position()}")
        if err != sl.ERROR_CODE.SUCCESS:
            print(f"err != sl.ERROR_CODE.SUCCESS")
            return None
        if ignore_data:
            print("ignore_data")
            return None
        ...
        if self.image:
            if self.concatenate_images:
                ...
                if config["droid"]["collector"]["use_one_side_camera"]:
                    data_dict["image"] = {
