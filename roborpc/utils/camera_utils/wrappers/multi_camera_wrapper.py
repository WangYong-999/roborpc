...
from droid.utils.camera_utils.camera_readers.zed_camera import gather_zed_cameras
from droid.utils.camera_utils.camera_readers.realsense_camera import gather_realsense_cameras
from droid.utils.camera_utils.info import get_camera_type
...
class MultiCameraWrapper:
    def __init__(self, camera_kwargs={}):
        ...
        all_cameras = zed_cameras + realsense_cameras
    ...
    def start_recording(self, recording_folderpath):
        svo_subdir = os.path.join(recording_folderpath, "SVO")
        mp4_subdir = os.path.join(recording_folderpath, "MP4")
        if config["droid"]["collector"]["save_data_mode"] == "H5_SVO":
            if not os.path.isdir(svo_subdir):
                os.makedirs(svo_subdir)
        if config["droid"]["collector"]["save_data_mode"] == "H5_MP4":
            if not os.path.isdir(mp4_subdir):
                os.makedirs(mp4_subdir)
        for cam in self.camera_dict.values():
            filepath = os.path.join(svo_subdir, cam.serial_number + ".svo")
